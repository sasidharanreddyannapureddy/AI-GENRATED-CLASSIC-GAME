...
export default function SnakeGame({ onScoreChange }: { onScoreChange: (score: number) => void }) {
  ...
  const handleDirection = useCallback((newDir: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT') => {
    ...
    switch (newDir) {
      case 'UP': if (directionRef.current.y !== 1) setDirection({ x: 0, y: -1 }); break;
      case 'DOWN': if (directionRef.current.y !== -1) setDirection({ x: 0, y: 1 }); break;
      case 'LEFT': if (directionRef.current.x !== 1) setDirection({ x: -1, y: 0 }); break;
      case 'RIGHT': if (directionRef.current.x !== -1) setDirection({ x: 1, y: 0 }); break;
    }
  }, [gameOver, isPaused]);
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="relative group p-4 bg-white/50 rounded-3xl shadow-xl backdrop-blur-sm border border-white/60">
        <div 
          className="relative overflow-hidden rounded-2xl shadow-inner border-4 border-green-800 game-board touch-none"
      ...
      <div className="mt-6 sm:hidden grid grid-cols-3 gap-2 w-full max-w-[220px] mx-auto">
        <div />
        <button 
          onClick={() => handleDirection('UP')}
          className="bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-md border-b-4 border-gray-300 flex items-center justify-center active:translate-y-1 active:border-b-0"
        >
        ...
        <button 
          ...
          className="bg-blue-100/80 backdrop-blur-md p-4 rounded-2xl shadow-md border-b-4 border-blue-300 flex items-center justify-center active:translate-y-1 active:border-b-0"
        >
          {isPaused ? <Play size={24} className="text-blue-600" /> : <Pause size={24} className="text-blue-600" />}
        </button>
        <button 
